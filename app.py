...
# Top 5 products
top_products = df.nlargest(5, "total_sales")[
...
# Monthly Sales
monthly_sales = df[monthly_sales_cols].sum()
st.caption(":blue[Overall Monthly Sales Trend]")
...
top_products_on_category = filtered_df.nlargest(5, "total_sales")[
    ["product_name", "category", "total_sales"]
]
...
st.caption(
    ":blue[Visualization of monthly sales of all the produts belonging to specific category.]"
)
st.line_chart(category_monthly_sales)
st.caption(
    ":blue[ Tabular representation of monthly sales of all the produts belonging to a specific category. ]"
)
